<template>
    <div>
        <h2 class="uppercase text-xs font-semibold text-gray-400 mb-6">RECENT ARTICLES</h2>

        <ul class="space-y-8">
            <li v-for="(article, id) in displayedArticles" :key="id">
                <NuxtLink class="group flex items-center justify-between border-b border-gray-800" target="_blank"
                    :to="article.url">
                    <article>
                        <div
                            class="relative z-10 order-first mb-3 flex items-center text-sm text-gray-400 dark:text-gray-500 pl-3.5">
                            <span aria-hidden="true" class="absolute inset-y-0 left-0 flex items-center">
        ...
        <div class="flex items-center justify-center mt-6 text-sm">
            <UButton color="gray" label="All Articles &rarr;" to="/articles" variant="link" />
        </div>
    </div>
</template>
