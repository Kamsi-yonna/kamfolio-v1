<template>
    <div ref="headerRef" class="animate-pulse fixed top-0 w-full z-50" :style="styles">
        <nav class="mx-auto px-4 sm:px-6 lg:px-8 max-w-2xl">
            <ul class="navbar">
                <li v-for="item in items" :key="item.path">
                    <UTooltip :text="item.name" :ui="{ popper: { strategy: 'absolute', arrow: 'true' } }">
                        <ULink active-class="text-primary dark:text-primary-400" class="navbar-ul" :to="item.path">
                            <Icon aria-hidden="true" class="w-5 h-5 z-10" :name="item.icon" />
                            <span v-if="$route.path === item.path" class="active-link"></span>
                            <span v-if="$route.path === item.path" class="active-link-circle"></span>
                ...
                <li class="flex-1"></li>
                <li>
                    <UTooltip text="Toggle theme" :ui="{ popper: { strategy: 'absolute', arrow: 'true' } }">
                        <button
